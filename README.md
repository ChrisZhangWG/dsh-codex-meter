...
```
● Session ¥0.52 · Today ¥1.25 · Balance ¥1.07
```

| 段 | 含义 | 刷新 |
| --- | --- | --- |
| `●` | 状态点（绿=可用 / 红=错误） | — |
| `Session ¥0.52` | 本会话费用（宿主回放会话日志，按官方价格表计价，含峰谷） | 15s（窗口可见时） |
| `Today ¥1.25` | 今日已消费（配置平台 token 后为**官方数据**；否则 `≈` 本地估算） | 60s（随余额轮询） |
| `Balance ¥1.07` | DeepSeek API 剩余余额（官方 `/user/balance`，实时准确） | 60s |

特性：

- 点击胶囊：立即刷新
- **窗口隐藏/最小化时暂停轮询**，恢复可见立即补刷；数值无变化时不触发重渲染
- **平台 token 过期/失效时**，Today 前显示黄色警示 `!`（`!Today ≈¥X`），不会静默退回估算
