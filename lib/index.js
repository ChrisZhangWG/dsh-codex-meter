...
import { fetchTodayUsageAnalysis } from "./official-usage.js";
        ...
        try {
          ...
          if (platformHit !== void 0) {
            try {
              ...
              try {
                todayAnalysis = await fetchTodayUsageAnalysis(platformHit.value, { timeoutMs: TIMEOUT_MS });
              } catch (error) {
                ctx.logger.warn("dsh-codex-meter: official token analysis is temporarily unavailable");
                ctx.logger.warn(error);
              }
            } catch (error) {
