...
//
// Click opens the DeepSeek platform usage page (usage details + top-up).
// Polling pauses while the window is hidden and only
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CodexMeter(props) {
			...
			// token 失效/出错时在 Today 前显示 "!" 告警（warn 色）。
			...
			const segments = [
				jsx("span", {
					...
					key: "session",
					children: [
						jsx("span", { style: label, children: "S" }),
				...
				jsx("span", {
					...
					key: "today",
					children: [
						...(tokenBroken ? [jsx("span", { style: warnBang, children: "!" })] : []),
						jsx("span", { style: label, children: "T" }),
						jsx("span", {
							style: todaySource === "official"
								? value
								: tokenBroken
									? { ...value, ...warn }
									: { ...value, ...estimate },
				...
				jsx("span", {
					...
					key: "balance",
					children: [
						jsx("span", { style: label, children: "B" }),
						jsx("span", {
							style: phase === "error"
			...
			return jsx("div", {
				...
				onClick: () => {
					// 打开 DeepSeek 平台用量页（明细 + 充值）。桌面端会把
					// window.open 的外链交给系统默认浏览器打开。
